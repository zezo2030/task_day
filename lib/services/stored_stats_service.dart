...
class StoredStatsService {
  ...
  static Future<void> _calculateAndStoreDailyStats(DateTime date) async {
    try {
      ...
      // حساب السلاسل التفصيلية (المعقدة)
      final habitStreaks = await _calculateDetailedHabitStreaks(habits);
  ...
  static Future<Map<String, int>> _calculateDetailedHabitStreaks(
    List<HabitModel> habits,
  ) async {
    final Map<String, int> streaks = {};

    for (final habit in habits) {
      final streak = await _calculateHabitHistoricalStreak(habit);
  ...
  static double _calculateHabitsScore(List<HabitModel> habits, DateTime date) {
    ...
    for (final habit in habits) {
      ...
      if (isCompletedToday) {
        ...
        final streak = QuickStatsService.getCurrentLongestStreak([habit]);
